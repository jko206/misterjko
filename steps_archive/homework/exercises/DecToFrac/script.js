...
/*global $, FracAddSub, randomBetween, PreMath, 
...
(function($global){
	$global.FracAddSub = (function(){
		/* list of constants and whatnot */
		var exName = 'FracAddSub';
		...
		function renderProblem(){
			...
			$('#lhs').html(str);
			renderMath();
			$('#FracAddSub-container .input').text('');
			$('#whole').focus();
		...
		return {
			...
			submitResponse: function(){
				...
				if(CAN_SUBMIT && (whole || numer || denom)){
					...
					window.processResponse('FracAddSub', isCorrect, t, probResp, level, skillSet, FIRST_TRY);
					FIRST_TRY = false;
				}
			},
			...
			renderUI: function($main){
				...
				var checkAnswer = $('<div></div>')
						...
						.text('Check Answer')
						.attr('id', 'FracAddSub-enter-target')
						.click(function(){
							FracAddSub.submitResponse();
						})
						.css({
						    position: 'absolute',
						    bottom: '10px',										
				...
				var obj = this;
				var loadProblemBtn = $(
							'<div id="load-FracAddSub-problem" class="button load-problem-button">'
							+ 'New Problem</div>'
						).css({
							position: 'absolute',
						    bottom: '10px',
