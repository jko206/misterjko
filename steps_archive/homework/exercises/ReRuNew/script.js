...
/*global $, ReRuNewNew, randomBetween, PreMath, 
...
(function($global){
	$global.ReRuNewNew = (function(){
		/* list of constants and whatnot */
		var exName = 'ReRuNewNew';
		...
		return {
			...
			submitResponse: function(){
				...
				if(CAN_SUBMIT && input){
					...
					window.processResponse('ReRuNewNew', isCorrect, t, input, level, skillSet, FIRST_TRY);
					FIRST_TRY = false;
				}
			},
			...
			renderUI: function($main){
				...
				var checkAnswer = $('<div></div>')
						...
						.text('Check Answer')
						.attr('id', 'ReRuNewNew-enter-target')
						.click(function(){
							ReRuNewNew.submitResponse();
						}).css({
						    position: 'absolute',
						    bottom: '10px',										
						    right: '10px',
				...
				var obj = this;
				var loadProblemBtn = $(
							'<div id="load-ReRuNewNew-problem" class="button load-problem-button">'
							+ 'New Problem</div>'
						).css({
							position: 'absolute',
						    bottom: '10px',
