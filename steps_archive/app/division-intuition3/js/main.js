...
-be able to load videos via ajax
-keep track of videos watched
-better interface
-be able to set cost per video
-make alert box
-Make remainders optional
-be able to do decimal exercises. 
-make more reference, like x10, x20, ... , x90, x100, x200, ... , x900, etc. 
-speed bonus
...
(function(){
	window.settingsObj = {
		...
		pointsPerCoin : 0,
		init: function(){
			this.dividendMin = docCookies.hasItem('dividendMin') ?
				parseInt(docCookies.getItem('dividendMin')) : 1000;
			this.dividendMax = docCookies.hasItem('dividendMax') ? 
				parseInt(docCookies.getItem('dividendMax')) : 9999;
			this.divisorMin = docCookies.hasItem('divisorMin') ?
				parseInt(docCookies.getItem('divisorMin')) : 10;
			this.divisorMax = docCookies.hasItem('divisorMax') ? 
				parseInt(docCookies.getItem('divisorMax')) : 99;
			this.crutchLevel = docCookies.hasItem('crutchLevel') ?
				parseInt(docCookies.getItem('crutchLevel')) : 1;
			this.points = docCookies.hasItem('points') ?
				parseInt(docCookies.getItem('points')) : 0;
			this.coins = docCookies.hasItem('coins') ? 
				parseInt(docCookies.getItem('coins')) : 0;
			this.pointsPerCoin = docCookies.hasItem('pointsPerCoin') ?
				parseInt(docCookies.getItem('pointsPerCoin')) : 5;
		},
		setDividendMinMax: function(min, max){
			this.dividendMin = parseInt(min);
			this.dividendMax = parseInt(max);
			docCookies.setItem('dividendMin', min);
			docCookies.setItem('dividendMax', max);
		},
		setDivisorMinMax: function(min, max){
			this.divisorMin = parseInt(min);
			this.divisorMax = parseInt(max);
			docCookies.setItem('divisorMin', min);
			docCookies.setItem('divisorMax', max);
		},
		setCrutchLevel: function(level){
			this.crutchLevel = parseInt(level);
			docCookies.setItem('crutchLevel', level);
		},
		changePoints: function(difference){
			var p = this.points + parseInt(difference);
			this.setPoints(p);
		},
		setPoints: function(p){
			this.points = p;
			docCookies.setItem('points', this.points, 31536e3, '/', 'stepsmath.co.kr');
		},
		changeCoins: function(d){
			this.coins += parseInt(d);
			docCookies.setItem('coins', this.coins, 31536e3, '/', 'stepsmath.co.kr');
			$('#coins').text(this.coins);
		},
		setPointsPerCoin: function(pointsPerCoin){
			this.pointsPerCoin = parseInt(pointsPerCoin);
			docCookies.setItem('pointsPerCoin', this.pointsPerCoin, 31536e3, '/', 'stepsmath.co.kr');
		}
	}
	window.settingsObj.init();
}());
...
$(document).ready(function(){
	...
	$('#check-progress').click(function(){
		var currentSnippet = window.currentSnippet;
		var dividend = currentSnippet.find('.dividend').text();
		var divisor = currentSnippet.find('.divisor').text();
		var quotient = currentSnippet.find('.quotient').text();
		var product = currentSnippet.find('.product').text();
		var remainder = currentSnippet.find('.remainder').text();
		dividend = parseInt(dividend);
		divisor = parseInt(divisor);
		quotient = parseInt(quotient);
		product = parseInt(product);
		remainder = parseInt(remainder);
		var globalQ = ($('#dividend').text() / $('#divisor').text());
		
		var qIsCorrect = quotient <= globalQ;
		var pIsCorrect = product == divisor * quotient;
		var rIsCorrect = dividend - (product) == remainder;
		// if input makes sense, add snippet with the remaining
		if(qIsCorrect && rIsCorrect && pIsCorrect && remainder == 0){
			alert('so how many did each person get?');
		} else if(qIsCorrect && rIsCorrect && pIsCorrect){
			addSnippet(remainder, divisor);
		} else {
			alert('try again!');
		}
	});
	...
	$('#show-controls').click(function(){
		if(!$('#controls').is(':visible')){
			$('#dividend-min').val(window.settingsObj.dividendMin);
			$('#dividend-max').val(window.settingsObj.dividendMax);
			$('#divisor-min').val(window.settingsObj.divisorMin);
			$('#divisor-max').val(window.settingsObj.divisorMax);
			$('#crutch-level').val(window.settingsObj.crutchLevel);
			$('#points-per-coin').val(window.settingsObj.pointsPerCoin);
		}
		$('#controls').toggle();
	});
	...
	$('#submit-settings').click(function(){
		...
		if(pw == data){
			$('#password').val('');
			var dividendMin = parseInt($('#dividend-min').val());
			var dividendMax = parseInt($('#dividend-max').val());
			var divisorMin = parseInt($('#divisor-min').val());
			var divisorMax = parseInt($('#divisor-max').val());
			var crutchLevel = parseInt($('#crutch-level').val());
			var changePoints = parseInt($('#change-points').val());
			var changeCoins = parseInt($('#change-coins').val());
			var pointsPerCoin = parseInt($('#points-per-coin').val());
			if(dividendMin && dividendMax) 
				window.settingsObj.setDividendMinMax(dividendMin, dividendMax);
			if(divisorMin && divisorMax) 
				window.settingsObj.setDivisorMinMax(divisorMin, divisorMax);
			if(divisorMin || divisorMax || dividendMin || dividendMax){
				newProblem();
			}
				
			if(crutchLevel) window.settingsObj.setCrutchLevel(crutchLevel);
			if(changePoints) window.settingsObj.changePoints(changePoints);
			if(changeCoins) window.settingsObj.changePoints(changeCoins);
			if(pointsPerCoin) window.settingsObj.setPointsPerCoin(pointsPerCoin);
			$('#controls').hide();
			renderPointBoxes();
			renderPoints();
			newProblem();
	...
	$('#load-problem').click(function(){
		var dividend = parseInt($('#dividend-to-load').val());
		var divisor = parseInt($('#divisor-to-load').val());
		var quotient = dividend / divisor;
		var isInt = quotient == parseInt(quotient);
		if(!isInt){
			alert('몫이 자연수가 아니게 됩니다');
		} else {
			loadProblem(dividend, divisor, quotient);
			$('#controls').hide();
		}
	...
	$('body').on('keydown', '.input', function(e){
		if(e.which == 27){
			...
			if($(this).hasClass('remainder')){
				$('#check-progress').click();
			} else if($(this).attr('id') == 'solution'){
				$('#check-solution').click();
			}
		}
	});
});
...
function renderPoints(){
	var p = window.settingsObj.points;
	$('.checkmark').each(function(i){
		if(i < p){
			$(this).addClass('checked');
		}
	});	
	var price = window.settingsObj.pointsPerCoin;
	var c = Math.floor(p / price);
	p = p % price;
	window.settingsObj.setPoints(p);
	if(c > 0){
		setTimeout(function(){
			window.settingsObj.changeCoins(c);
			$('.checked').removeClass('checked');
			// make the light flash
		}, 500);
	}
	$('#coins').text(window.settingsObj.coins);
}
...
function renderPointBoxes(){
	$('#point-boxes').empty();
	var p = window.settingsObj.pointsPerCoin;
