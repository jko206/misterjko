...
// From: https://www.hackerrank.com/challenges/crush/problem
function arrayManipulation(n, queries) {
  function combineIVs(iv1, iv2){
    /*
      Ways to overlap:
      int1: []  int2: ()
      - []__()
      ...
      - ( [ ) ]
    */
    function intsOverlap(iv1, iv2){
      
    }
    function combineOverlap(iv1, iv2){
      
    }
    function combineDiscrete(iv1, iv2){
      
      
    }
    let int1 = iv1.intervals;
    let int2 = iv2.intervals;
    let val1 = iv1.values;
    let val2 = iv2.values;
    
    let int1_i = int1[0];
    let int1_f = int1[int1.length - 1];
    let int2_i = int2[0];
    let int2_f = int2[int2.length - 1];
  }
  // initializa
  let intVals = queries.map(e=>{
    let [i, j, val] = e;
    return {
      intervals: [i - 1, j],
      values: [val],
    ...
    // sort by interval
    return a.intervals[0] - b.intervals[0]; 
  });
  
  let max = -Infinity;
  while(intVals.length > 1){
    let iv1 = intVals.shift();
    let iv2 = intVals.shift();
    let int3 = combineIVs(iv1, iv2);
    intVals.push(int3);
  }
