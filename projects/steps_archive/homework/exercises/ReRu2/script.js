...
(function($global){
	$global.ReRu2 = (function(){
		...
		var startPos, markBox, depth; //operation
		
		/*
		'<div class="rr2-mark-box">'
			'<div class="rr2-rule-num"></div>'
			'<div class="rr2-mark"></div>'
		'</div>'
		*/
		function renderProblem(){
			$('#rr2-mark-boxes').empty();
			for(var i = startPos; i < startPos + 100; i++){
				var clone = markBox.clone(true);
				if(i == answer) clone.find('.rr2-mark').addClass('ans-mark');
				if(i % 10 == 0){ // 1 cm marks
					clone.find('.rr2-mark').addClass('rr2-10');
					clone.find('.rr2-rule-num').text(i/10);
				} else if(i % 5 == 0){ // 5mm marks
					clone.find('.rr2-mark').addClass('rr2-5');
				} else { // 1mm marks
					clone.find('.rr2-mark').addClass('rr2-1');
				}
				$('#rr2-mark-boxes').append(clone);
				
				var offset = (answer - startPos) * 5 + 2.5;
				$('#rr2-input-wrapper').css('left', offset);
			}
			
			PointsSetting.startBonusTimer();
		}
		function checkAnswer(input){
			return answer == input * depth;
		}
		...
		return {
			...
			loadProblem: function(toLoad){
                ...
                depth = Math.ceil(Math.random() * level);
                depth = Math.pow(10, depth);
				//render the problem
				renderProblem();
			...
			renderUI: function($main){
				...
				$main.append($(
					'<div id="rr2-ruler">'
					+	'<div id="rr2-mark-boxes">'	
					+   '</div>'
					+	'<div id="rr2-input-wrapper">'
					+		'<div id="rr2-pointer">'
					+		'</div>'
					+		'<div id="rr2-input" class="input" contenteditable>'
					+		'</div>'
					+	'</div>'
				   +'</div>'
				));
			...
			getExName: function(lang = 'eng'){
				if(lang == 'eng') return 'Reading Ruler2';
				else if(lang == 'kor') return '자 읽기2';
			}
		};
	}());
